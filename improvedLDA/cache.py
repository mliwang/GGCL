...
#    
    return rec
def LRU(train,N,predict,userfiled="clientIP",timefiled="timestamp",itemfiled="URL"):
    """
    train为用户点击序列
    N为最终每个用户需要留存的内容数目
    predict为需要关注的用户
    """
    dd=dict(train.groupby([userfiled]).apply(lambda x:list(x.sort_values(by=timefiled , ascending=True)[itemfiled])))#每个用户最近点击内容的序列
    rec={}
#    sum_popularity= getPoplarity(P)
    ...
    return rec
def FIFO(train,N,predict,userfiled="clientIP",timefiled="timestamp",itemfiled="URL"):
    """
    train为用户点击序列
    N为最终每个用户需要留存的内容数目
    predict为需要关注的用户
